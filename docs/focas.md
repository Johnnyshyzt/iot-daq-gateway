...
4. 启动网关（控制台或自行注册的 Windows 服务）。适配器会调用 `cnc_allclibhndl3`；之后每轮扫描读状态 / 报警 / 程序号，失败则释放句柄并在后续扫描重连。

本仓库不提供 MSI/服务安装程序。现场若需开机自启，可用 `sc.exe` 或 NSSM 把已发布的 `Gateway.Host.exe` 注册为 Windows 服务（超出 FOCAS 绑定范围，此处仅作提示）。
