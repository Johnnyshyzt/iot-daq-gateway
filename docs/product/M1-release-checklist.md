...
- [x] 发版：推送 `v*` 标签后，用与 CI `pack-win-x64` 相同的脚本把 `iot-daq-gateway-<version>-win-x64.zip` 挂到 GitHub Release（不含 `Fwlib64.dll`）。`git tag vX.Y.Z && git push origin vX.Y.Z`。合并 PR 不会自动打标签
