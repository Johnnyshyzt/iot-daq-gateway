...
Config Studio 是商业前端，与网关同机嵌入。本页只冻结页面树和每页依赖的 Management API，方便开源契约与商业 UI 对齐。交互细节与视觉不在本仓库。
...
| 点位 | `/points` | 按设备编辑点表；CSV/Excel 导入导出在商业仓库完成，结果仍 `PUT` 成 PointSet | `GET/PUT /api/v1/config/points/{deviceId}` | engineer、admin |
...
列表来自 `GET /api/v1/config/devices`。新建与编辑提交完整 Device 文档到 `PUT /api/v1/config/devices/{id}`。`metadata.id` 与路径一致。适配器只有 `fanuc.fake` 与 `fanuc.focas`。

连接测试调用 `POST /api/v1/devices/{id}/test`，针对草稿里的连接参数。Fake 恒为成功；FOCAS 做真实握手。测试不发布配置。

删除设备时同时去掉该设备的 PointSet 草稿。
...
CSV/Excel 导入导出是商业功能。导入结果必须变成符合 `point-set.schema.json` 的文档再写入草稿，不能另存一套表。

### 北向 MQTT `/sinks/mqtt`

编辑唯一的 MqttSink（`sinks/mqtt.yaml`）。认证只填环境变量名，不填密码。Topic 模板默认 `daq/{site}/{deviceId}/{point}`，状态主题默认 `daq/{site}/{deviceId}/$status`。

JSON 预览只在浏览器里用模板和一条样例点拼出载荷，不连 Broker。载荷字段与运行时 MQTT JSON 一致：`gatewayId`、`site`、`deviceId`、`point`、`value`、`quality`、`unit`、`ts`。v1 Gateway 资源没有单独的 `gateway.id`；预览里的 `gatewayId` 在加载器落地前仍对应今天单文件配置中的 `gateway.id`，落地时用 `metadata.siteId` 作为 `site`。
...
观测是内存中的短窗口，不是时序库。日志尾读取 Runtime 滚动文件（`logs/gateway-yyyyMMdd.log`）。设备状态取值与 `AdapterStatus` 一致：`online`、`degraded`、`offline`。
